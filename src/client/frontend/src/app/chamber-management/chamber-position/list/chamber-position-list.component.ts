...
@Component({
  templateUrl: './chamber-position-list.component.html',
})
export class ChamberPositionListComponent implements OnInit, DoCheck {
  public id: string;
  public know: any;
  ...
  async getKnow() {
    try {
      ...
      let data = await this.chamberPositionService.get(this.id, this.filter.search === '' ? _.omit(this.filter, 'search') : this.filter).toPromise();
  ...
  async delete(id: string): Promise<any> {
    try {
      ...
      if (confirmed) {
        this.tableLoading = true;
        await this.chamberPositionService.delete(this.id, id).toPromise();
