...
async function index(req, res, next) {
  ...
  const rules = {
    ...
    parentId: 'nullable|integer|min:1|exists:ActivityCategories,id'
  };
  ...
  try {
    const result = await modelHelper.findAll('ActivityCategories', [], filter, []);
...
async function show(req, res, next) {
  ...
  try {
    const result = await models.ActivityCategories.findById(req.params.categoryId);
...
async function create(req, res, next) {
  ...
  const rules = {
    ...
    parentId: 'nullable|integer|exists:ActivityCategories,id',
  ...
  try {
    ...
    const result = await models.ActivityCategories.create(input, {transaction: t});
    await t.commit();
...
async function update(req, res, next) {
  ...
  try {
    if (_.isNaN(parseInt(input.parentId))) delete input.parentId;
    const result = await models.ActivityCategories.findById(req.params.categoryId, {transaction: t});
...
async function destroy(req, res, next) {
  ...
  const t = await models.sequelize.transaction();
  try {
    const result = await models.ActivityCategories.findOne({where: {id: req.params.categoryId}, transaction: t});
    if (_.isNull(result)) throw new MainError('common', 'notFound');
